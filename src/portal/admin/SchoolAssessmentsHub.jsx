...
export default function SchoolAssessmentsHub({ allStudents = [], onSwitchToGazette }) {
  ...
  return (
    <div className="space-y-4 text-slate-100">
      ...
      {alertMsg && (
        <div className={`p-3 rounded-xl text-xs font-bold flex items-center justify-between gap-2 border ${
          ...
          <div className="flex items-center gap-2">
            <CheckCircle2 size={16} className={alertMsg.type === 'success' ? 'text-emerald-400' : 'text-rose-400'} />
      ...
      {/* Header Info & Public Access Card */}
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xs">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-teal-500/15 border border-teal-500/30 text-teal-300 text-[11px] font-bold">
              <ShieldCheck size={12} className="text-teal-400" />
              <span>School Examination & Assessment Authority</span>
            </div>
            <h2 className="text-lg sm:text-xl font-black text-white tracking-tight m-0">
              School Assessments & Pre-Board Hub
            </h2>
            <p className="text-xs text-slate-400 max-w-2xl leading-relaxed m-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="px-3 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-slate-950 font-black text-xs flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
            >
            ...
            {onSwitchToGazette && (
              <button
                ...
                className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer"
              >
            ...
            <button
              ...
              disabled={loading}
              className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 transition-all cursor-pointer disabled:opacity-50"
        ...
        <div className="mt-4 pt-3 border-t border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-slate-400 font-bold text-[11px]">Student Result Lookup:</span>
            <code className="bg-slate-950 px-2 py-0.5 rounded border border-slate-800 text-teal-300 font-mono text-[11px]">
              /results
            </code>
            <span className="text-[10px] text-slate-500">Searchable by Reg No / Roll No / Form No</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleCopyPublicLink}
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-[11px] font-bold flex items-center gap-1 cursor-pointer transition-all"
            >
            ...
            <a
              ...
              className="px-2.5 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-slate-950 text-[11px] font-black flex items-center gap-1 cursor-pointer transition-all shadow-xs"
            >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs font-bold text-slate-400 flex items-center gap-1">
          <Sparkles size={13} className="text-amber-400" /> Instant Presets:
        </span>
        {PRESET_EVALUATIONS.map((preset, idx) => (
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 hover:border-teal-500/50 text-slate-300 hover:text-white text-xs font-semibold transition-all cursor-pointer"
          >
            + {preset.title}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
        {evaluations.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="bg-slate-900/90 border border-slate-800 hover:border-slate-700 rounded-2xl p-4 flex flex-col justify-between gap-3 transition-all relative overflow-hidden"
            >
              {/* Top Accent Line */}
              <div className={`absolute top-0 left-0 right-0 h-1 ${
                isTeacherOpen ? 'bg-teal-500' : 'bg-slate-700'
              }`} />
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-[10px] font-black tracking-wider uppercase px-2 py-0.5 rounded bg-teal-950 text-teal-300 border border-teal-800/60">
                ...
                <div>
                  <div className="text-[11px] text-slate-300 flex items-center gap-1.5">
                    <span className="text-slate-500 font-semibold">Evaluation Type:</span>
                    <strong className="text-teal-300 font-bold">{item.evalType}</strong>
                  </div>
                  {item.description && (
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-800 text-[11px]">
                  <div className="flex items-center gap-1 flex-wrap">
                    ...
                    {(Array.isArray(item.classes) ? item.classes : ['10th', '11th', '12th']).map(cls => (
                      <span key={cls} className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-200 font-bold text-[10px]">
                        {cls}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    <span className="text-slate-500 font-semibold">Student Status:</span>
                  ...
                  <div className="flex items-center justify-between text-[10px] text-slate-400 pt-1">
                    <span>Subject Max: <strong>{item.maxMarks || 100}</strong></span>
                    <span>Min Pass: <strong>{item.minMarks || 36} ({Math.round(((item.minMarks || 36)/(item.maxMarks || 100))*100)}%)</strong></span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2.5 border-t border-slate-800 flex items-center justify-between gap-2">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wide transition-all cursor-pointer ${
                    isTeacherOpen
                      ? 'bg-teal-950 text-teal-300 border border-teal-800/60 hover:bg-teal-900/60'
                      : 'bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-750'
                  }`}
                  title="Toggle Teacher Marks Entry Portal open or locked"
                >
                  {isTeacherOpen ? <Unlock size={11} className="text-teal-400" /> : <Lock size={11} />}
                ...
                >
                  {isPublished ? <Eye size={11} className="text-emerald-400" /> : <EyeOff size={11} />}
      ...
      {evaluations.length === 0 && !loading && (
        <div className="bg-slate-900/50 border border-dashed border-slate-800 rounded-2xl p-8 text-center space-y-3">
          ...
          <h4 className="text-sm font-bold text-slate-300">No School Assessments Configured</h4>
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-slate-950 font-black text-xs transition-all cursor-pointer inline-flex items-center gap-1.5"
          >
            <Sparkles size={14} />
      ...
      {/* Create / Edit Modal */}
      {showModal && (
        <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-xl w-full p-4 sm:p-6 shadow-2xl space-y-4 my-auto animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleSaveModal} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white focus:outline-none focus:border-teal-500 font-medium"
                  />
                </div>
                      ...
                      return (
                        <button
                          ...
                          className={`flex-1 py-1.5 px-2 rounded-xl text-xs font-black transition-all cursor-pointer border ${
                            isSelected
                              ? 'bg-teal-600 text-slate-950 border-teal-500 shadow-xs'
              ...
              {/* STUDENT STATUS CONTROL (Requested Feature) */}
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 rounded-xl border text-left transition-all cursor-pointer flex items-center gap-2 ${
                          isChecked
                            ? 'bg-teal-950/60 border-teal-500/60 text-teal-200 shadow-2xs'
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-950 border border-slate-800 cursor-pointer">
                  <input
                    ...
                    className="rounded border-slate-700 text-teal-600 focus:ring-0 w-4 h-4 cursor-pointer"
                  />
                  <div>
                    <span className="block text-xs font-bold text-slate-200">Teacher Award Entry</span>
                    <span className="text-[10px] text-slate-400">Enable marks submission on teacher portal</span>
                  </div>
                </label>
                ...
                <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-950 border border-slate-800 cursor-pointer">
                  <input
                    ...
                    className="rounded border-slate-700 text-emerald-600 focus:ring-0 w-4 h-4 cursor-pointer"
                  />
                  <div>
                    <span className="block text-xs font-bold text-slate-200">Student Result Lookup</span>
                    <span className="text-[10px] text-slate-400">Make results searchable at /results</span>
                  </div>
                </label>
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-800">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="px-5 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-slate-950 text-xs font-black flex items-center gap-1.5 shadow-md cursor-pointer disabled:opacity-50"
                >
