  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="absolute left-6 bottom-6 text-left text-white z-20 pointer-events-none max-w-[60%]">
        <div className="bg-black/40 px-2 py-1 rounded-md backdrop-blur-sm inline-block max-w-[95%] whitespace-normal">
          <h3 className="text-xs sm:text-sm font-bold text-teal-300 inline">{slides[index].title || 'School'}</h3>
          <span className="text-[10px] sm:text-xs italic text-white/90 inline ml-2">{slides[index].caption || 'the beacon of knowledge'}</span>
        </div>
      </div>
    </div>
  );
