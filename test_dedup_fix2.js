...
sortedActive.forEach((a) => {
  ...
  // Check 1: Roll No collision for same class+session
  if (rollNo) {
  ...
  // Check 2: Same Student Name + Father Name for same class+session
  if (!isDuplicate && nameClean && nameClean.length > 2 && fnameClean && fnameClean.length > 1) {
    const nameKey = `name_${scope}_${nameClean}_${fnameClean.slice(0, 8)}`;
...
console.log('Sorted Kept active count:', keptList.length);
console.log('Sorted Pruned active count:', prunedList.length);
console.log('\nPruned list sample (should ONLY be unapproved duplicate drafts):');
prunedList.forEach(p => console.log(p));

console.log('\n=== APPROVED COUNT BY CLASS IN KEPT LIST ===');
const breakdown = {};
keptList.forEach(k => {
  if (k.roll) {
    const key = `${k.cls} (${k.sess})`;
