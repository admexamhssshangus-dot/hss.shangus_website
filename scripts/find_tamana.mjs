...
async function findStudentInAdmissions() {
  const token = await getAccessToken();
  
  // Search admissions collection using runQuery
  const queryPayload = {
    structuredQuery: {
  ...
  const res = await restRequest('POST', ':runQuery', queryPayload, token);
  if (Array.isArray(res)) {
    const matching = [];
    for (const item of res) {
      if (item.document) {
        const d = decodeDoc(item.document);
        const str = JSON.stringify(d).toLowerCase();
        if (str.includes('2401010000200028') || str.includes('tamana')) {
          matching.push(d);
        }
      }
    }
    console.log('Matching admissions records count:', matching.length);
    for (const m of matching) {
      console.log('Full Admission doc:', JSON.stringify(m, null, 2));
    }
  }
}

findStudentInAdmissions().catch(console.error);
