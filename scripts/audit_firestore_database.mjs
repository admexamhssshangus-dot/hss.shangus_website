...
async function listCollections(token) {
  const res = await restRequest('POST', ':listCollectionIds', {}, token);
...
async function run() {
  ...
  const collIds = await listCollections(token);
  console.log(`Found ${collIds.length} collections:`, collIds);

  const report = [];

  for (const coll of collIds) {
    ...
    report.push({
      ...
      docIdsSample: docs.slice(0, 5).map(d => d.name.split('/').pop())
    });
  }

  console.table(report);

  // Detailed inspect of formStructure
  if (collIds.includes('formStructure')) {
    console.log('\n📋 Deep inspection of "formStructure":');
    const formDocs = await fetchCollectionDocs('formStructure', token);
    console.log(`Total fields in formStructure: ${formDocs.length}`);
    const fieldsSummary = formDocs.map(d => {
      const id = d.name.split('/').pop();
      const data = parseFirestoreFields(d.fields);
      return { id, fieldName: data.fieldName || data['Field Name'], type: data.fieldType || data['Field Type'], required: data.required || data['Is Required?'] };
    });
    console.log('Sample formStructure definitions:', fieldsSummary.slice(0, 15));
  }
}
