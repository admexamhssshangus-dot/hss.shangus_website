...
async function run() {
  ...
  console.log(`📋 Total documents in formStructure: ${formDocs.length}\n`);

  const list = formDocs.map(d => {
    const docId = d.name.split('/').pop();
    const data = parseFirestoreFields(d.fields);
    return {
      docId,
      fieldName: data.fieldName || data['Field Name'] || docId,
      fieldType: data.fieldType || data['Field Type'] || 'text',
      required: data.required !== undefined ? data.required : (data['Is Required?'] === 'TRUE' || data['Is Required?'] === true),
      options: data.options || data['Options / Range / Length'] || '',
      helpText: data.helpText || data['Help Text'] || ''
    };
  });

  list.sort((a, b) => a.fieldName.localeCompare(b.fieldName));

  console.log('--- ALL FORM STRUCTURE FIELDS ---');
  list.forEach((f, idx) => {
    const optStr = String(f.options || '');
    console.log(`${idx + 1}. [${f.docId}] -> "${f.fieldName}" | Type: ${f.fieldType} | Req: ${f.required} | Opts: ${optStr ? optStr.slice(0, 30) : 'none'}`);
  });
}
