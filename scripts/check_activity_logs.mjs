...
async function checkLogs() {
  ...
  console.log('📡 Fetching adminActivityLogs...');
  const logs = await fetchCollectionDocs('adminActivityLogs', token);
  console.log(`Total logs: ${logs.length}`);

  const forms = ['250218', '250398', '250407', '250558', '250271'];
  for (const log of logs) {
    const dStr = JSON.stringify(log);
    for (const f of forms) {
      if (dStr.includes(f)) {
        console.log(`Log matching Form ${f}: actionType=${log.actionType} | title=${log.actionTitle} | details=${log.details}`);
      }
    }
  }
}
