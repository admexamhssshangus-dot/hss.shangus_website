...
async function simulate() {
  ...
  const cohort11th = admDocs.filter(s => sameCohort(s, '2025-26', '11th'));
  console.log(`Cohort 11th (2025-26) students count: ${cohort11th.length}`);

  const polDoc = pracDocs.find(d => d.id === '11th_Political Science_Pre-Board Test_2025-26');
  const evsDoc = pracDocs.find(d => d.id === '11th_Environmental Science_Pre-Board Test_2025-26');

  console.log("PolDoc records count:", polDoc?.records?.length);
  console.log("EvsDoc records count:", evsDoc?.records?.length);

  // Check how many students in 11th match PolDoc and EvsDoc
  let polMatches = 0;
  let polAbsent = 0;
  let polMarks = 0;
  let polUnmatched = 0;

  for (const st of cohort11th) {
    const id = recordIdentity(st);
    const subStr = String(st['Subjects to be taken in Class 11th'] || st['Subjects Studied in Class 11th'] || st.subjects || st['Stream / Subjects'] || '').toLowerCase();
    const isPolEnrolled = subStr.includes('pol');
    const rec = polDoc.records.find(r => matchStudentRecord(r, st, id));
    if (rec) {
      polMatches++;
      ...
      if (isAb) {
        polAbsent++;
        if (isPolEnrolled) {
          console.log(`Pol Absent student: Roll:${id.roll} | Form:${id.form} | Name:${st["Student's Name (as per school records)"] || st.name} | DocName:${rec.name} | Mark:${rawM}`);
        }
      } else {
        polMarks++;
      }
    } else if (isPolEnrolled) {
      polUnmatched++;
      console.log(`Pol ENROLLED BUT NO RECORD MATCHED: Roll:${id.roll} | Form:${id.form} | Name:${st["Student's Name (as per school records)"] || st.name}`);
    }
  }

  console.log(`\nPol summary: total cohort: ${cohort11th.length}, matched in polDoc: ${polMatches} (with marks: ${polMarks}, absent: ${polAbsent}), enrolled but unmatched: ${polUnmatched}`);
  ...
  for (const st of cohort11th) {
    const id = recordIdentity(st);
    const subStr = String(st['Subjects to be taken in Class 11th'] || st['Subjects Studied in Class 11th'] || st.subjects || st['Stream / Subjects'] || '').toLowerCase();
    const isEvsEnrolled = subStr.includes('env') || subStr.includes('evs');
    const rec = evsDoc.records.find(r => matchStudentRecord(r, st, id));
    if (rec) {
      evsMatches++;
      const rawM = rec.totalMarks ?? rec.practicalMarks;
      const isAb = /^(a|ab|absent)$/i.test(String(rawM).trim());
      if (isAb) {
        evsAbsent++;
        if (isEvsEnrolled) {
          console.log(`EVS Absent student: Roll:${id.roll} | Form:${id.form} | Name:${st["Student's Name (as per school records)"] || st.name} | DocName:${rec.name} | Mark:${rawM}`);
        }
      } else {
        evsMarks++;
      }
    } else if (isEvsEnrolled) {
      evsUnmatched++;
      console.log(`EVS ENROLLED BUT NO RECORD MATCHED: Roll:${id.roll} | Form:${id.form} | Name:${st["Student's Name (as per school records)"] || st.name}`);
    }
  }
  console.log(`\nEVS summary: total cohort: ${cohort11th.length}, matched in evsDoc: ${evsMatches} (with marks: ${evsMarks}, absent: ${evsAbsent}), enrolled but unmatched: ${evsUnmatched}`);
}
