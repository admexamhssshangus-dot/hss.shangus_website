...
async function run() {
  ...
  console.log('🚀 Checking masterRegisters for base64 photo bloat...');
  ...
  for (const doc of mrDocs) {
    ...
    // Remove the base64 root keys from masterRegisters doc
    const cleanFields = {};
    for (const [k, v] of Object.entries(fields)) {
    ...
    await restRequest('PATCH', `/masterRegisters/${encodeURIComponent(docId)}`, { fields: cleanFields }, token);
    cleanedCount++;
    console.log(`✅ Removed base64 keys [${base64Keys.join(', ')}] from masterRegisters doc: ${docId}`);
  }

  console.log(`\n🎉 Finished cleaning masterRegisters! Total docs relieved of photo bloat: ${cleanedCount}`);
}
