...
async function run() {
  ...
  console.log('🔍 Checking for duplicate user accounts or blank admissions...');
  const admDocs = await fetchCollectionDocs('admissions', token);
  let blankCount = 0;
  admDocs.forEach(d => {
    const data = parseFirestoreFields(d.fields);
    if (!data.name && !data["Student's Name (as per school records)"] && !data["Student's Name"] && !data.formNo && !data['Form Number']) {
      blankCount++;
    }
  });

  console.log(`Admissions total: ${admDocs.length}, Completely blank/null docs: ${blankCount}`);
}
