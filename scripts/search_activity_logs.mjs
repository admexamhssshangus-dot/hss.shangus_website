...
async function searchLogs() {
  ...
  const relevant = logs.filter(l => {
    ...
    return s.includes('kali') || s.includes('political') || s.includes('english') || s.includes('environmental') || s.includes('pre-board');
  });

  console.log(`Found ${relevant.length} relevant logs:`);
  relevant.sort((a, b) => (a.timestamp || a.date || a.createdAt || '').localeCompare(b.timestamp || b.date || b.createdAt || ''));
  relevant.forEach(l => {
    console.log(`[${l.timestamp || l.date || l.createdAt}] ${l.actionTitle || l.activityType || l.type}: ${l.details || l.description || l.action}`);
  });
}
