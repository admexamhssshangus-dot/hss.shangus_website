...
async function inspectSiteDocs() {
  const token = await getAccessToken();
  const noticesRes = await restRequest('GET', '/site/notices', null, token);
  console.log("=== FIRESTORE /site/notices ===");
  console.log(parseFirestoreFields(noticesRes.fields));

  const slideshowRes = await restRequest('GET', '/site/slideshow', null, token);
  console.log("=== FIRESTORE /site/slideshow ===");
  console.log(parseFirestoreFields(slideshowRes.fields));
}

inspectSiteDocs().catch(console.error);
