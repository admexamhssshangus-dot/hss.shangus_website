...
async function run() {
  ...
  const doc = await restRequest('GET', '/studentPhotos/photo_2301003001220025', null, token);
  const data = parseFirestoreFields(doc.fields);
  console.log('Doc photo_2301003001220025 fields:');
  for (const [k, v] of Object.entries(data)) {
    const preview = typeof v === 'string' ? (v.length > 50 ? `${v.slice(0, 50)}... (length ${v.length})` : v) : v;
