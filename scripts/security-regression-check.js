...
assert(/function verifiedStaffIdentity\(\)[\s\S]{0,160}email_verified == true/.test(rules) && /function isBootstrapAdmin\(\)[\s\S]{0,130}verifiedStaffIdentity/.test(rules), 'Bootstrap admin requires verified identity');
...
assert(/match \/certificateNumberLocks\/\{certificateNo\}[\s\S]{0,520}allow delete: if false;/.test(rules), 'Certificate-number locks can be deleted and reused');
assert(/hasAny\(\['certificateNo', 'regNo', 'regKey', 'createdAt'\]\)/.test(rules), 'Certificate lock ownership fields are mutable');
...
assert(/documentId in \['faculty', 'recycle_bin'\] && canUseAny/.test(rules), 'Private CMS documents require an assigned administrator');
...
assert(/certificateNumberLocks/.test(publicStudentLookup) && /issuedDocuments/.test(publicStudentLookup) && /loadSource/.test(publicStudentLookup), 'Verification requires authoritative issuance and a current source');
assert(/where\(new FieldPath\(field\), '==', value\)/.test(read('netlify/functions/lib/publicRecords.js')) && /query\.limit\(20\)/.test(read('netlify/functions/lib/publicRecords.js')), 'Public verification queries must be exact and bounded');
